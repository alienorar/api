...
async function getPhotos() {
    try {
        const response = await fetch(`https://jsonplaceholder.typicode.com/comments?_page=${page}&_limit=${limit_items}`)
...
function displayPhotos() {
    ...
    <div class="card-body">
    <p>${item.body}</p></div>
    <div class="card-footer"> <h6>${item.name}</h6>
    <h5>${item.email}</h5></div></div>`
